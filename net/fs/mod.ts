...
const serveFile: Transform<string, HandlerFunc> = (pathname) =>
    async ({ respondWith }) => {
...
const serveDir: Transform<string, HandlerFunc> = (pathname) =>
    async ({ respondWith }) => {
        let body = "";
...
const serve: BiConsumerAsync<RequestContext, FileServerInit> = async (
    ctx,
    { filepath, url: { href, pathname } },
) => {
    ...
    return !pathname.endsWith("/")
        ? await handleRedirect({ href })(ctx)
        : await serveDir(filepath + pathname)(ctx);
};
...
const fileServerOptions: BiTransform<
    string,
    Pick<Request, "url">,
    FileServerInit
> = (filepath, { url }) => ({ filepath, url: new URL(url) });

export const stripPrefix: BiTransform<string, Handler, Handler> = (
    prefix,
    handler,
) => (new class implements Handler {
    async serveHttp(
        ctx: RequestContext,
        { href, pathname } = new URL(ctx.request.url),
    ): Promise<void> {
...
export const _strip = (prefi: string, handler: Handler) => {
    return handler;
};

// class FileServer implements Handler { }

export const fileServer: Transform<string, Handler> = (
    pathname: string,
) => (new class implements Handler {
    async serveHttp(ctx: RequestContext): Promise<void> {
        // return await ctx.respondWith(new Response());
